...
//  LLM
//
//  Port of https://github.com/ml-explore/mlx-lm/blob/main/mlx_lm/models/glm_moe_dsa.py
...
class GLMMoeDSAMLP: Module, UnaryLayer {
    let hiddenSize: Int
    ...
    init(_ config: GLMMoeDSAConfiguration, hiddenSize: Int? = nil, intermediateSize: Int? = nil) {
...
class GLMMoeDSAGate: Module {
    ...
    init(_ config: GLMMoeDSAConfiguration) {
        guard let nRoutedExperts = config.nRoutedExperts else {
            fatalError("GLMMoeDSAGate requires nRoutedExperts")
        }
...
class GLMMoeDSAMoE: Module, UnaryLayer {
    let numExpertsPerTok: Int
    let gate: GLMMoeDSAGate

    @ModuleInfo(key: "switch_mlp") var switchMLP: SwitchGLU
    @ModuleInfo(key: "shared_experts") var sharedExperts: GLMMoeDSAMLP?

    init(_ config: GLMMoeDSAConfiguration) {
        guard let nRoutedExperts = config.nRoutedExperts else {
            fatalError("GLMMoeDSAMoE requires nRoutedExperts")
        }

        self.numExpertsPerTok = config.numExpertsPerTok
        self.gate = GLMMoeDSAGate(config)
        ...
        if let shared = config.nSharedExperts, shared > 0 {
            ...
            _sharedExperts.wrappedValue = GLMMoeDSAMLP(
...
class GLMMoeDSADecoderLayer: Module {
    @ModuleInfo(key: "self_attn") var attention: GLMMoeDSAAttention
    let mlp: UnaryLayer

    @ModuleInfo(key: "input_layernorm") var inputLayerNorm: RMSNorm
    @ModuleInfo(key: "post_attention_layernorm") var postAttentionLayerNorm: RMSNorm

    init(_ config: GLMMoeDSAConfiguration, layerIdx: Int) {
        _attention.wrappedValue = GLMMoeDSAAttention(config)
...
public class GLMMoeDSAModelInner: Module, LayerPartitionable, StreamableMoE {
    @ModuleInfo(key: "embed_tokens") var embedTokens: Embedding

    let layers: [GLMMoeDSADecoderLayer]
    let norm: RMSNorm
    ...
    init(_ config: GLMMoeDSAConfiguration) {
        ...
        self.layers = (0 ..< config.hiddenLayers)
            .map { idx in
                GLMMoeDSADecoderLayer(config, layerIdx: idx)
            }
...
public struct GLMMoeDSAConfiguration: Codable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        let container: KeyedDecodingContainer<GLMMoeDSAConfiguration.CodingKeys> =
            try decoder.container(keyedBy: GLMMoeDSAConfiguration.CodingKeys.self)
        ...
        self.partialRotaryFactor = try container.decodeIfPresent(Float.self, forKey: .partialRotaryFactor) ?? 1.0
...
extension GLMMoeDSAModel: LoRAModel {
